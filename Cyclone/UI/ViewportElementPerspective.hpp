#pragma once

// Cyclone UI
#include "Cyclone/UI/ViewportElement.hpp"
#include "Cyclone/UI/Tool/BaseTool.hpp"

namespace Cyclone::Core {
	class LevelInterface;
}

namespace Cyclone::UI
{
	class ViewportElementPerspective : public ViewportElement
	{
		static constexpr float kMouseSensitivity = 0.01f;
		static constexpr float kKeyboardSensitivity = 5.0f;
		static constexpr float kCameraDollySensitivity = 5.0f;
		static constexpr float kHorizontalFOV = DirectX::XM_PIDIV2;

	public:
		ViewportElementPerspective( DXGI_FORMAT inBackBufferFormat, DXGI_FORMAT inDepthBufferFormat, const DirectX::XMVECTORF32 inClearColor, bool inAntialiasing ) : ViewportElement( inBackBufferFormat, inDepthBufferFormat, inClearColor, inAntialiasing ) {}

		void UpdateNavigation( float inDeltaTime, Cyclone::Core::LevelInterface *inLevelInterface );
		void UpdateTools( float inDeltaTime, Cyclone::Core::LevelInterface *inLevelInterface, const Tool::ToolChanger &inTools );
		void DrawGizmos( float inDeltaTime, Cyclone::Core::LevelInterface *inLevelInterface, const Tool::ToolChanger &inTools );
		void Render( ID3D11DeviceContext3 *inDeviceContext, Cyclone::Core::LevelInterface *inLevelInterface, const Tool::ToolChanger &inTools );
	};
}
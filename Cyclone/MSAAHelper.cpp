...
MSAAHelper::MSAAHelper( DXGI_FORMAT backBufferFormat,
	...
	m_height( 0 )
{
	if ( sampleCount < 1 || sampleCount > D3D11_MAX_MULTISAMPLE_SAMPLE_COUNT )
	{
	...
	switch ( m_depthBufferFormat ) {
		case DXGI_FORMAT_D32_FLOAT:
			m_depthBufferFormatTypeless = DXGI_FORMAT_R32_TYPELESS;
			m_depthBufferFormatBindable = DXGI_FORMAT_R32_FLOAT;
			break;

		case DXGI_FORMAT_D16_UNORM:
			m_depthBufferFormatTypeless = DXGI_FORMAT_R16_TYPELESS;
			m_depthBufferFormatBindable = DXGI_FORMAT_R16_UNORM;
			break;
		...
		case DXGI_FORMAT_D32_FLOAT_S8X24_UINT:
			m_depthBufferFormatTypeless = DXGI_FORMAT_R32G8X24_TYPELESS;
			m_depthBufferFormatBindable = DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS;
			break;

		case DXGI_FORMAT_UNKNOWN:
			m_depthBufferFormatTypeless = DXGI_FORMAT_UNKNOWN;
			m_depthBufferFormatBindable = DXGI_FORMAT_UNKNOWN;
			break;
		
		default:
			throw std::out_of_range( "Unknown depth buffer format." );
	}
}

void MSAAHelper::ValidateSampleCount( ID3D11Device *device )
{
	{
	...
	for ( m_sampleCount = m_targetSampleCount; m_sampleCount > 0; m_sampleCount-- )
	{
		UINT levels = 0;
...
void MSAAHelper::SetDevice( _In_ ID3D11Device *device )
{
	...
	ValidateSampleCount( device );

	m_device = device;
}

void MSAAHelper::SetSampleCount( unsigned int sampleCount )
{
	m_targetSampleCount = sampleCount;

	ValidateSampleCount( m_device.Get() );
...
void MSAAHelper::SizeResources( size_t width, size_t height )
{
	...
	if ( m_depthBufferFormat != DXGI_FORMAT_UNKNOWN )
	{
		...
		const CD3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc(
			m_sampleCount > 1 ? D3D11_DSV_DIMENSION_TEXTURE2DMS : D3D11_DSV_DIMENSION_TEXTURE2D,
			m_depthBufferFormat
		);
		...
		SetDebugObjectName( m_depthStencilSRV.Get(), "MSAA Depth/Stencil SRV" );
	}
